...
class GCNBlock(torch.nn.Module):
    def __init__(
        ...
        self.layers.append(
            ...
            ProcessorLayer(self.in_channels, self.out_channels, 
                        aggregate_type='sum')
        )
        for _ in range(self.n_layers - 1):
            self.layers.append(
                # GCNConv(self.out_channels, self.out_channels, dtype=torch.float32)
                ProcessorLayer(self.out_channels, self.out_channels, 
                            aggregate_type='sum')
            )
...
class PARC(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # )
        # self.input_fields = torch.nn.Sequential(
        # self.input_fields = ProcessorLayer(self.n_fields + self.n_bcfields, self.n_hiddenfields, 
        #                 aggregate_type='sum')
            # LayerNorm(self.n_hiddenfields),
            # torch.nn.ReLU(),
            # ProcessorLayer(self.n_hiddenfields, self.n_hiddenfields),
            # torch.nn.ReLU()
        # )

        self.derivative_solver = DerivativeSolver(
            in_channels = self.n_hiddenfields + self.n_fields + self.n_bcfields,
    ...
    def forward(self, F_initial, mesh_features, edge_index, F_bc : OptTensor = None):
        ...
        for timestep in range(self.n_timesteps):
            if self.n_bcfields > 0:
                ...
                F_temp = F_current
            # F_temp = self.input_fields(F_temp, edge_index)
